...
def save_csv(id_list, label_list, csvpath):
    """
    Save predictions to a CSV file.

    Args:
        id_list (list): List of IDs.
        label_list (list): List of predicted labels.
        csvpath (str): Path to save the CSV file.

    Returns:
        None
    """
    df = pd.DataFrame({'id' : id_list , 'intent' : label_list})
    df.to_csv(csvpath , index = False)
    print(f"Predictions saved to {csvpath}")
    return    
...
def Parse():
    ...
    """
    my_parser = ArgumentParser(description="Training intent classifier for ADL 2022 Fall HW1 Problem1")
    #data
    my_parser.add_argument("--train_data", type=str, default="data/intent/train.json", help="Path to the json file of training data")
    my_parser.add_argument("--val_data", type=str, default="data/intent/eval.json", help="Path to the json file of validation data")
    my_parser.add_argument("--test_data", type=str, default="data/intent/test.json", help="Path to the json file of test data")
    my_parser.add_argument("--label_idx", type=str, default="cache/intent/intent2idx.json", help="Path to the json file of labelindex")
    #word embedding
    my_parser.add_argument("--word_vector", type=str, default="cache/intent/embeddings.pt", help="Path to the word embedding data")
    my_parser.add_argument("--vocab", type=str, default="cache/intent/vocab.pkl", help="Path to the Class Vocab containing the token information")
    my_parser.add_argument("--sequence_len", type=int, default="32", help="max length of sequence tokens")
    ...
    my_parser.add_argument('--epoch' , type = int , default = 256, help="the number of epoch")

    #model save & load
    my_parser.add_argument("--model_dir", type=str, default= "p1_intent_model", help="directory for saving model")
    my_parser.add_argument("--load_model", type=str, default= "intent_best.pth", help="filename of model")
